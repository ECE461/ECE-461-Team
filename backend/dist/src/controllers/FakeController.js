"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.FakeController = void 0;
const PackageID_1 = require("../models/package/PackageID");
const PackageMetadata_1 = require("../models/package/PackageMetadata");
const PackageData_1 = require("../models/package/PackageData");
const PackageCommandController_1 = require("./PackageCommandController");
const PackageQueryController_1 = require("./PackageQueryController");
const PackageQuery_1 = require("../models/package/PackageQuery");
const PackageRegex_1 = require("../models/package/PackageRegex");
const Package_1 = require("../models/package/Package");
const User_1 = require("../models/authentication/User");
const PackageHistoryEntry_1 = require("../models/package/PackageHistoryEntry");
const PackageName_1 = require("../models/package/PackageName");
const AuthenticationRequest_1 = require("../models/authentication/AuthenticationRequest");
class FakeController {
    static getPackagesByQuery(req, res) {
        // Query Validation
        if (!PackageQuery_1.PackageQuery.isValidQuery(req)) {
            res.status(400).json(PackageQueryController_1.PackageQueryController.MSG_INVALID);
            return;
        }
        const m1 = new PackageMetadata_1.PackageMetadata("React", "2.2.3");
        const m2 = new PackageMetadata_1.PackageMetadata("Lodash", "1.2.4");
        const m3 = new PackageMetadata_1.PackageMetadata("React", "1.2.5");
        const response = [m1.getJson(), m2.getJson(), m3.getJson()];
        res.status(200).json(response);
    }
    static getPackagesByRegex(req, res) {
        if (!PackageRegex_1.PackageRegex.isValidRegexRequest(req)) {
            res.status(400).json(PackageQueryController_1.PackageQueryController.MSG_INVALID);
            return;
        }
        const m1 = new PackageMetadata_1.PackageMetadata("React", "2.2.3");
        const m2 = new PackageMetadata_1.PackageMetadata("Lodash", "1.2.4");
        const m3 = new PackageMetadata_1.PackageMetadata("React", "1.2.5");
        const response = [m1.getJson(), m2.getJson(), m3.getJson()];
        res.status(200).json(response);
    }
    static getPackageById(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!PackageID_1.PackageID.isValidGetByIdRequest(req)) {
                res.status(400).json(PackageQueryController_1.PackageQueryController.MSG_INVALID);
                return;
            }
            const metadata = new PackageMetadata_1.PackageMetadata("React", "1.2.3");
            const data = yield PackageData_1.PackageData.create(FakeController.FAKE_CONTENT, "");
            const pckg = new Package_1.Package(metadata, data);
            res.status(200).json(pckg.getJson());
        });
    }
    static getRating(req, res) {
        if (!PackageID_1.PackageID.isValidGetByIdRequest(req)) {
            res.status(400).json(PackageQueryController_1.PackageQueryController.MSG_INVALID);
            return;
        }
        const fakeRes = {
            "BusFactor": 0,
            "Correctness": 0,
            "RampUp": 0,
            "ResponsiveMaintainer": 0,
            "LicenseScore": 0,
            "GoodPinningPractice": 0,
            "PullRequest": 0,
            "NetScore": 0
        };
        res.status(200).json(fakeRes);
    }
    static getPackageHistoryByName(req, res) {
        if (!PackageName_1.PackageName.isValidGetByNameRequest(req)) {
            res.status(400).json(PackageQueryController_1.PackageQueryController.MSG_INVALID);
            return;
        }
        const user = new User_1.User("James Davis", true);
        const metadata = new PackageMetadata_1.PackageMetadata("React", "2.2.3");
        const entry1 = new PackageHistoryEntry_1.PackageHistoryEntry(user, new Date().toISOString(), metadata, PackageHistoryEntry_1.PackageHistoryEntry.Action.CREATE);
        const entry2 = new PackageHistoryEntry_1.PackageHistoryEntry(user, new Date().toISOString(), metadata, PackageHistoryEntry_1.PackageHistoryEntry.Action.RATE);
        const entry3 = new PackageHistoryEntry_1.PackageHistoryEntry(user, new Date().toISOString(), metadata, PackageHistoryEntry_1.PackageHistoryEntry.Action.UPDATE);
        const entry4 = new PackageHistoryEntry_1.PackageHistoryEntry(user, new Date().toISOString(), metadata, PackageHistoryEntry_1.PackageHistoryEntry.Action.DOWNLOAD);
        const fakeRes = [entry4.getJson(), entry3.getJson(), entry2.getJson(), entry1.getJson()];
        res.status(200).json(fakeRes);
    }
    static updatePackage(req, res) {
        if (!PackageData_1.PackageData.isValidUploadOrUpdateRequest(req.body)) {
            res.status(400).json(PackageCommandController_1.PackageCommandController.MSG_INVALID);
            return;
        }
        res.status(200).send({ message: "Version is updated." });
    }
    static uploadPackage(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            // Check format:
            if (!PackageData_1.PackageData.isValidUploadOrUpdateRequest(req.body)) {
                res.status(400).json(PackageCommandController_1.PackageCommandController.MSG_INVALID);
                return;
            }
            const metadata = new PackageMetadata_1.PackageMetadata("React", "1.2.3");
            const data = yield PackageData_1.PackageData.create(FakeController.FAKE_CONTENT, "");
            const pckg = new Package_1.Package(metadata, data);
            res.status(201).json(pckg.getJson());
        });
    }
    static reset(req, res) {
        // TODO: Add validity check
        res.status(200).send({ message: "Registry is reset." });
    }
    static deletePackageById(req, res) {
        if (!PackageID_1.PackageID.isValidGetByIdRequest(req)) {
            res.status(400).json(PackageCommandController_1.PackageCommandController.MSG_INVALID);
            return;
        }
        res.status(200).send({ message: "Package is deleted." });
    }
    static deletePackageByName(req, res) {
        if (!PackageName_1.PackageName.isValidGetByNameRequest(req)) {
            res.status(400).json(PackageCommandController_1.PackageCommandController.MSG_INVALID);
            return;
        }
        res.status(200).send({ message: "Package is deleted." });
    }
    static createAccessToken(req, res) {
        if (!AuthenticationRequest_1.AuthenticationRequest.isValidRequest(req)) {
            res.status(400).json(PackageCommandController_1.PackageCommandController.MSG_INVALID);
            return;
        }
        const fakeRes = {
            value: '"bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c"'
        };
        res.status(200).json(fakeRes);
    }
}
exports.FakeController = FakeController;
FakeController.FAKE_CONTENT = "";
